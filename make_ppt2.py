...
def clear_and_set_text(shape, new_text):
    """Clear all text in shape and set new text, preserving basic formatting from first run"""
    ...
    if saved_font_color:
        run.font.color.rgb = saved_font_color
    if saved_font_name:
...
for shape in slide.shapes:
    ...
    if shape.name.startswith("Docer"):
        if "系统简介" in text and len(text) < 10:
            ...
            clear_and_set_text(shape, "12个模块：节点/路径增删查、最短路径、文件读写、图形化展示")
...
for shape in slide.shapes:
    if not shape.has_text_frame:
        continue
    if shape.name == "矩形 4":
        clear_and_set_text(shape, "系统功能演示与测试用例\n\n1. 启动系统：输入口令1进入主菜单\n2. 添加节点：图书馆、教学楼、食堂、宿舍\n3. 添加路径：设置各节点间路径长度\n4. 查询最短路径：自动计算最优路线\n5. 查看邻接矩阵：直观展示图结构\n6. 保存/加载地图：数据持久化到文件")
