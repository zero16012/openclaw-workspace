...
// Fix "展" "示" split - merge by removing the split marker
s1 = s1.replace(/展<\/a:t><\/a:r><a:r><a:rPr><\/a:rPr><a:t>示/g, '展示');
s1 = rep(s1, '绩管理系统', '校园导航系统');
...
s2 = s2.replace(/子模的详细设计与实现解<\/a:t><\/a:r><a:r><a:rPr><\/a:rPr><a:t>说/g, '子模块详细设计与实现解说');
...
s4 = s4.replace(/系统简<\/a:t><\/a:r><a:r><a:rPr><\/a:rPr><a:t>介/g, '系统概述');
s4 = s4.replace(/代码量与复杂<\/a:t><\/a:r><a:r><a:rPr><\/a:rPr><a:t>度/g, '代码量与复杂度');
s4 = s4.replace(/数据结构选<\/a:t><\/a:r><a:r><a:rPr><\/a:rPr><a:t>型/g, '数据结构选型');
s4 = s4.replace(/功能模块个<\/a:t><\/a:r><a:r><a:rPr><\/a:rPr><a:t>数/g, '功能模块总数');
...
s6 = rep(s6, '手绘或用工具绘制模块关系图（如 "学生信息管理系统" 分为 "录入模块→存储模块→统计模块"）。', '主菜单控制13个子模块，循环调用各功能');
s6 = rep(s6, '说明模块交互（如 "录入模块调用存储模块的结构体数组，统计模块读取数据后排序"）。', '选择数字进入对应功能模块，执行完毕后返回主菜单');
...
s10 = rep(s10, '显示记', '双重循环初始化矩阵');
s10 = rep(s10, '修改记', '循环输入节点名称');
s10 = rep(s10, '插入新记', '校验编号 + 双向赋值');
s10 = rep(s10, '删除记', 'dist+pre+visit初始化');
s10 = s10.replace(/录<\/a:t>/g, '</a:t>'); // clean up orphan "录"
...
for (const entry of vZip.getEntries()) {
  if (entry.entryName.startsWith('ppt/slides/slide') && entry.entryName.endsWith('.xml')) {
    ...
    if (texts) {
      ...
      console.log(`\n${entry.entryName}:`);
      console.log(`  ${content.substring(0, 180)}`);

      // Check for any remaining old text
      const remainingOld = ['学生成绩管理', '张三', '2501123121', '录入模块', '统计模块', '不及格'];
      for (const old of remainingOld) {
        if (content.includes(old)) {
          console.log(`  ⚠️ Remaining old text: "${old}"`);
        }
      }
    }
  }
